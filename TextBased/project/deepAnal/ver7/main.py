...
flags.DEFINE_integer("speech_emb_size", 100, "speech_emb_size")
...
flags.DEFINE_integer("speech_size", 56, "speech_size")
flags.DEFINE_float("learning_rate", 0.001, "learning_rate")
flags.DEFINE_float("min_learning_rate", 0.0002, "min_learning_rate")
...
flags.DEFINE_string("train_data_speech_path", './Data/train_new_40000_speech.txt', "train_data_speech_path")
...
flags.DEFINE_string("test_data_speech_path", './Data/train_10000_speech.txt', "test_data_speech_path")
flags.DEFINE_string("parse_data_path", './Data/taiyi_semi_test.csv', "parse_data_path")
flags.DEFINE_string("parse_data_speech_path", './Data/taiyi_semi_test_speech.txt', "parse_data_speech_path")
...
flags.DEFINE_bool("CRF", True, "has_CRF")
FLAGS = flags.FLAGS
...
def nostop():
    ...
    args.train_data_label_path = './Data/train_new_20000_label.txt'
    args.train_data_speech_path = './Data/train_new_20000_speech.txt'
    args.mode = '3'
    ...
    with tf.Session() as sess:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            for e in range(args.epochs):
                ...
                if args.lr_decreased == True:
                    args.learning_rate = args.learning_rate - dataloader.lr_decay
                for _ in range(dataloader.num_batch):
                    x, x_pre, x_pos, speech, y, length, weight = dataloader.next_batch()
                    counter += 1
                    start = time.time()
                    feed = {model.inputs: x,
                            model.inputs_pre: x_pre,
                            model.inputs_pos: x_pos,
                            model.inputs_speech: speech,
                            model.targets: y,
...
def parse_test(args):
    write = False
    args.save_dir = '/home/luofeng/PycharmProjects/DFCompetition/deepAnal/ver7/save_ver7_100speechsize_CRF_newData'
    # args.save_dir = './save_2017-11-26_17classes_100epoches_150numsteps_withAttention_2weight'
    dataloader = Dataloder2(args=args)
    model = Model(args)

    if write:
        save_file_path = './Data/res1204_ver7_10batchsize_65epoches_continueTrainedOn20000.csv'
    ...
    with tf.Session() as sess:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            for i in range(len(dataloader.raw_data)):
                ...
                x, x_pre, x_post, speech, length = dataloader.create_one_data(dataloader.raw_data[i], dataloader.raw_speech[i])
                predict, _ = model.predict_class(sess, x, x_pre, x_post, speech, length)
        ...
        if write:
            csvFile.close()
    writeList('./Data/predict_list_ver7.txt', result_list)
def test(args):
    args.save_dir = '/home/luofeng/PycharmProjects/DFCompetition/deepAnal/ver7/save_ver7_100speechsize_CRF_newData'
    dataloader = Dataloder2(args=args)
    model = Model(args)
    with tf.Session() as sess:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            for i in range(dataloader.num_batch):
                ...
                x, x_pre, x_post, speech, y, length,weight = dataloader.next_batch()
                predict, probability = model.predict_class(sess, x, x_pre, x_post, speech, length)
                out1 = ''
                out2 = ''
                out3 = ''
                for i in range(min(len(content), len(predict))):
                    ...
                    out3 = out3+content_vector[i] + '[' + str(probability[i])+']'
                print content
                print ' '.join([str(j) for j in y[0]])
                print ' '.join([str(j) for j in predict])
                print ' '.join([str(j) for j in probability])
                print 'y:' + out1
                print 'p' + out2
                print 'pro' + out3
                print '**********'
