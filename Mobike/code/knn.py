...
#返回 欧式距离  （其实还可以返回南北方向距离,东西方向距离,曼哈顿距离,方向(-0.5:0.5)）
...
def training(trainfile = 'train.csv',testfile = 'test.csv',subfile = 'submission.csv' ,
    ...
    for rec in te1:
        # rec是测试集中的每一条记录，首先获得需要预测每条记录的关键信息
        iter1 += 1
        if iter1  % 10000== 0:
        ...
        #knn
        # 对于有用户记录(在train中有记录)的用户，使用历史记录的knn预测
        if user1 in user_habit_dict:
            # 对于此用户在train中所有的记录数遍历，同样获得关键的特征信息
            for eachAct in user_habit_dict[user1]:
                ...
                dis = loc_2_dis(start1, start2)   # 计算在train中每一条记录对应起点与本次起点的距离
                dis = min(dis, 1000)    #1000     # 距离太大，直接按照1000计算
                qidian= qidianquan * (dis / 100) ** 2 # 距离加权 dis/1000  *10

                detalaTime = abs(hour2 - hour1) if abs(hour2 - hour1) < 12 else 24 - abs(hour2 - hour1)   # 计算train中每一条记录与当前需要预测的时间差
                shijian= shijianquan * (detalaTime / 12 * 10) ** 2   # 时间加权

                dayType = isHoliday2 - isHoliday1                # 是否同为节假日
                jiejia= jiejiaquan * (dayType * 10) ** 2         # 节假日加权

                biType = int(biketype2) - int(biketype1)         # 是否是一种类型的车子
                bike= bikequan * (biType * 10) ** 2  #0.5        # biketype加权
                ...
                if end2 in hotLoc:
                    hotLoc[end2] += 1          # 对每个终点出现的次数进行记录
                else:
                    hotLoc[end2] = 1

                if end2 in result:
                    if result[end2] > score:
                        result[end2] = score     # 对于同一终点不同记录的得分，取最低的，也就是距离最近的.
                else:
                    result[end2] = score

            for each in hotLoc:
                result[each] = result[each] / (hotLoc[each]**zhishu)  # 每一个终点的得分除以此终点出现的次数

            for each in result:
                result[each] = math.sqrt(result[each])   # 开根号，类距离，求得最终每一个终点作为本条test记录预测结果的距离得分

        #利用test中的用户历史记录
        if user1 in user_habit_dict_test:
            resulttest = {}
            user_habit_dict_test[user1].sort(key = lambda x:x['data']*60*24+x['minute'])  # 按照时间顺序对test中对应用户记录进行排序
            xuhao = 0
            for i in range(len(user_habit_dict_test[user1])-1):
                if user_habit_dict_test[user1][i]['orderid'] == order1:
                    xuhao = i
                    resulttest[user_habit_dict_test[user1][i+1]['geohashed_start_loc']] = 21   # 当前记录的下一条记录起始地点赋值为21
...
if __name__ =="__main__":
    training('data/train.csv', 'data/test.csv', 'knnResult/submission.csv' )
    # training('data/train_small.csv', 'data/test_small.csv', 'knnResult/submission.csv')
